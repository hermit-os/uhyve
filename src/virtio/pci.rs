...
impl HeaderConf {
	...
	/// Reads fields from the configuration at offset `address` into `data`.
	/// The offset must point to valid bytes in the HeaderConf.
	...
	/// Writes `data` into the Header conf at the offset `address`.
	/// Returns `PciError` if the write is not aligned, or read only.
