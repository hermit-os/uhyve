...
impl NetDevCfg {
	pub const MAC_ADDRESS: u32 = get_offset!(DEVICE_CFG_START, NetDevCfg, mac).0;
	pub const MAC_ADDRESS_HIGH: u32 =
		ConfigAddress(get_offset!(DEVICE_CFG_START, NetDevCfg, mac).0 + 4).0;
	pub const MAC_ADDRESS_END: u32 = get_offset!(DEVICE_CFG_START, NetDevCfg, mac).0 + 6;
	pub const NET_STATUS: u32 = get_offset!(DEVICE_CFG_START, NetDevCfg, status).0;
	pub const MTU: u32 = get_offset!(DEVICE_CFG_START, NetDevCfg, mtu).0;
}
...
impl IsrStatus {
	pub const ISR_FLAGS: u32 = ConfigAddress::from_configuration_address(ISR_CFG_START as u32).0;
}
...
#[allow(dead_code)]
impl ComCfg {
	pub const DEVICE_FEATURE_SELECT: u32 =
		get_offset!(COMMON_CFG_START, Self, device_feature_select).0;

	pub const DEVICE_FEATURE: u32 = get_offset!(COMMON_CFG_START, Self, device_feature).0;

	pub const DRIVER_FEATURE_SELECT: u32 =
		get_offset!(COMMON_CFG_START, ComCfg, driver_feature_select).0;

	pub const DRIVER_FEATURE: u32 = get_offset!(COMMON_CFG_START, ComCfg, driver_feature).0;

	pub const CONFIG_MSIX_VECTOR: u32 = get_offset!(COMMON_CFG_START, ComCfg, config_msix_vector).0;

	pub const DEVICE_STATUS: u32 = get_offset!(COMMON_CFG_START, ComCfg, device_status).0;

	pub const CONFIG_GENERATION: u32 = get_offset!(COMMON_CFG_START, ComCfg, config_generation).0;

	pub const QUEUE_SELECT: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_select).0;

	pub const QUEUE_SIZE: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_size).0;

	pub const QUEUE_MSIX_VECTOR: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_msix_vector).0;

	pub const QUEUE_ENABLE: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_enable).0;

	pub const QUEUE_NOTIFY_OFFSET: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_notify_off).0;

	pub const QUEUE_DESC_LOW: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_desc).0;
	pub const QUEUE_DESC_HIGH: u32 =
		ConfigAddress(get_offset!(COMMON_CFG_START, ComCfg, queue_desc).0 + 4).0;

	pub const QUEUE_DRIVER_LOW: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_driver).0;
	pub const QUEUE_DRIVER_HIGH: u32 =
		ConfigAddress(get_offset!(COMMON_CFG_START, ComCfg, queue_driver).0 + 4).0;

	pub const QUEUE_DEVICE_LOW: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_device).0;
	pub const QUEUE_DEVICE_HIGH: u32 =
		ConfigAddress(get_offset!(COMMON_CFG_START, ComCfg, queue_device).0 + 4).0;

	pub const QUEUE_NOTIFY_DATA: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_notify_data).0;

	pub const QUEUE_RESET: u32 = get_offset!(COMMON_CFG_START, ComCfg, queue_reset).0;
