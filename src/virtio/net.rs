...
use crate::{
	...
	virtio::{
		...
		pci::{ConfigAddress, HeaderConf, MEM_NOTIFY_RX, MEM_NOTIFY_TX},
	},
};
...
pub(crate) fn wait_eventfd_with_timeout(fd: &EventFd, timeout: u16) -> io::Result<i32> {
...
impl VirtioNetPciDevice {
	pub fn new(
		interface_cfg: NetworkMode,
		guest_mmap: Arc<vm_memory::GuestMemoryMmap>,
	) -> VirtioNetPciDevice {
	...
	pub fn setup(&mut self, vm: &VmFd) {
		...
		vm.register_ioevent(
			...
			&IoEventAddress::Mmio(ConfigAddress(MEM_NOTIFY_RX).guest_address()),
			NoDatamatch,
		)
		.unwrap();
		...
		vm.register_ioevent(
			...
			&IoEventAddress::Mmio(ConfigAddress(MEM_NOTIFY_TX).guest_address()),
			NoDatamatch,
		)
		.unwrap();
		...
		self.tx_thread = Some({
			...
			thread::spawn(move || {
				...
				while !stop_threads.load(Ordering::Relaxed) {
					let read_cnt =
						wait_eventfd_with_timeout(&notify_evtfd_tx, UHYVE_NET_READ_TIMEOUT)
							.expect("Could not read eventfd. Is the file nonblocking?");
					if read_cnt > 0 {
						notify_evtfd_tx.read().unwrap();
