...
use crate::{
	...
	hypercall,
	linux::KVM,
	net::virtio::{ConfigAddress, offsets::*},
	params::Params,
					...
					VcpuExit::MmioWrite(addr, data) => {
						match ConfigAddress::from_guest_address(addr) {
							DEVICE_STATUS => {
								let mut virtio_device =
									self.peripherals.virtio_device.lock().unwrap();
								virtio_device.write_status(data, &self.peripherals);
							}
							...
							QUEUE_DESC => {
								...
								let mut virtio_device =
									self.peripherals.virtio_device.lock().unwrap();
								virtio_device.write_pfn(data, &self.peripherals.mem);
							}
							QUEUE_ENABLE => {
								let mut virtio_device =
									self.peripherals.virtio_device.lock().unwrap();
