					...
					VcpuExit::IoOut(port, addr) => {
						...
						if let Some(hypercall) = unsafe {
							...
							match hypercall {
								...
								_ => unreachable!("Attempted to run v2 hypercall over I/O ports!"),
							};
						} else {
							if let Some(s) = self.stats.as_mut() {
					...
					VcpuExit::MmioWrite(addr, data) => {
						{
							...
							warn!("addr: {addr:#x}, data_addr: {data_addr:p}");
							if let Some(hypercall) = unsafe {
								hypercall::address_to_hypercall_v2(
									&self.peripherals.mem,
									addr as u16,
								...
								// FIXME: Fix stats.
								//if let Some(s) = self.stats.as_mut() {
								//	s.increment_val(VmExit::Hypercall(v2::HypercallAddress::from(
								//		&hypercall,
								//	)))
								//}
