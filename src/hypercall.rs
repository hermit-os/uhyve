...
pub fn read(mem: &GuestMemoryMmap, sysread: &mut ReadParams, root_pt: GuestPhysAddr) {
	unsafe {
	...
	while bytes_written != syswrite.len {
		...
		unsafe {
			let step = libc::write(
				...
				peripherals
					.mem
					.get_host_address(GuestAddress::new(guest_phys_addr.as_u64()))
					.map_err(|e| match e {
