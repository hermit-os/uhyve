...
impl<VirtBackend: VirtualizationBackend> UhyveVm<VirtBackend> {
	pub fn new(kernel_path: PathBuf, mut params: Params) -> HypervisorResult<UhyveVm<VirtBackend>> {
		...
		let mut maybe_hermit_image = None;
			...
			Some(Format::Gzip) => {
				...
				let handle = match params.hermit_image_mode {
					...
					HermitImageMode::Internal => {
						match config::parse_tar(maybe_hermit_image.insert(buf_decompressed)) {
							Ok(handle) => handle,
							Err(e) => {
		...
		let maybe_hermit_image = maybe_hermit_image.map(|hermit_image| {
			load_hermit_image_to_mem(
				&hermit_image[..],
				&mut mem,
				(kernel_end_address - guest_address).align_up(crate::consts::PAGE_SIZE as u64),
			)
...
fn write_fdt_into_mem(
	mem: &MmapMemory,
	params: &Params,
	maybe_hermit_image: Option<core::ops::Range<GuestPhysAddr>>,
	...
	let mut fdt = Fdt::new(maybe_hermit_image)
		.unwrap()
...
fn load_hermit_image_to_mem(
	...
	let aligned_image_len = hermit_image.len().align_up(crate::consts::PAGE_SIZE);
