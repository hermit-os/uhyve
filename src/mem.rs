...
pub(crate) unsafe fn mem_as_slice(
	...
	if mem.check_range(guest_addr, len) {
		Ok(unsafe {
			std::slice::from_raw_parts_mut(mem.get_host_address(guest_addr).unwrap(), len)
		})
	} else {
...
pub(crate) unsafe fn mem_as_slice_virt(
	...
	let guest_addr =
		virt_to_phys(start, mem, root_pt).map_err(|_err| MemoryError::WrongMemoryError)?;
	unsafe { mem_as_slice(mem, guest_addr, len) }
}
...
/// - `addr` must follow the alignment rules for `T`.
#[allow(dead_code)]
pub(crate) unsafe fn mem_get_ref<T>(
	mem: &GuestMemoryMmap,
	addr: GuestPhysAddr,
