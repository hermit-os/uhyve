...
/// A virtual directory tree, possibly with links to on-host and virtual read-only files.
#[derive(Clone, Debug)]
pub enum Tree {
	File(File),
	Directory(Directory),
...
/// Returns the [`File`] corresponding to given a requested guest_path, if it can exist.
///
/// * `guest_path` - The guest path that is to be looked up in the directory.
pub fn resolve_guest_path(mut this: &Directory, guest_path: &[u8]) -> Option<File> {
	let guest_pathstr = prepare_guest_path(guest_path)?;
	for component in guest_pathstr.split('/') {
		let file = match this.get(component) {
			None => {
				debug!(
			...
			Some(Tree::Directory(subdir)) => {
				this = subdir;
				continue;
			}
			Some(Tree::File(file)) => file,
		};
	...
	core::iter::from_fn(move || {
		while let Some(mut top) = stack.pop() {
			match top.next() {
				...
				None => continue,
				Some(Tree::Directory(dir)) => {
					stack.push(dir.values());
				}
				Some(Tree::File(File::Virtual(_))) => {}
				Some(Tree::File(File::OnHost(hp))) => {
					stack.push(top);
	...
	core::iter::from_fn(move || {
		while let Some((prefix, mut top, mut marked)) = stack.pop() {
			match top.next() {
				...
				Some((component, Tree::Directory(dir))) => {
					let mut new_prefix = prefix.clone();
					new_prefix.push('/');
					new_prefix.push_str(component);
					stack.push((new_prefix, dir.iter(), false));
				}
				Some((_, Tree::File(File::Virtual(_)))) => {
					marked = true;
				}
				Some((component, Tree::File(File::OnHost(hp)))) => {
...
/// Insert a file (which might be an [`File::OnHost`] mount point) into the directory tree.
///
/// This fails if any parent of the specified `guest_path` is a file or mount point instead of a directory.
pub fn create_file(mut this: &mut Directory, guest_path: &[u8], file: File) -> bool {
	...
	let leaf = match it.next_back() {
		None | Some("") => {
			debug!("create_file invoked on empty path, rejecting...");
			return false;
		}
		Some(leaf) => leaf,
	};
	...
	for component in it {
		match this.entry(component.to_string().into_boxed_str()) {
			Entry::Vacant(vac) => {
				...
				this = match vac.insert(Tree::Directory(Directory::new())) {
					Tree::Directory(x) => x,
					_ => unreachable!(),
				};
			}
			Entry::Occupied(occ) => match occ.into_mut() {
				Tree::Directory(subdir) => {
					this = subdir;
				}
				Tree::File(_) => {
					debug!(
						"create_file {guest_pathstr:?}: Guest reguested to create a file inside of an already mapped file."
					);
					return false;
				}
			},
	...
	match this.entry(leaf.to_string().into_boxed_str()) {
		Entry::Vacant(vac) => {
			trace!("create_file {guest_pathstr:?} <- {file:?}");
			vac.insert(Tree::File(file));
			true
		}
		Entry::Occupied(_) => {
			debug!(
				"create_file {guest_pathstr:?}: Guest reguested to create a file, but it already exists"
			);
			false
		}
	}
...
///
/// Note that if `guest_path` points to a mount point (`File::Onhost`) or virtual file (`File::Virtual`),
...
pub fn unlink(mut this: &mut Directory, guest_path: &[u8]) -> Result<Option<PathBuf>, ()> {
	...
	for component in it {
		let file = match this.get_mut(component) {
			None => {
				debug!(
	...
	match this.entry(leaf.to_string().into_boxed_str()) {
		...
		Entry::Occupied(occ) => {
			...
			match occ.remove_entry() {
				(_, Tree::File(File::OnHost(oh))) => {
					ret = Some(oh);
				}
				(_, Tree::File(File::Virtual(_))) => {}
				(_, Tree::Directory(dir)) if dir.is_empty() => {}
				(leaf, Tree::Directory(dir)) => {
					debug!(
						"unlink {guest_pathstr:?}: Tried to unlink non-empty directory, rejecting..."
					);
					this.insert(leaf, Tree::Directory(dir));
					return Err(());
				}
			}
			Ok(ret)
