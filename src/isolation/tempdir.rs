...
/// parent directory in [`crate::isolation::landlock::initialize`], a sub-folder
/// called `uhyve-{uid}` (e.g. `/tmp/uhyve-1000`) is used to create temporary
/// directories under.
///
...
pub fn create_temp_dir(dir_path: Option<&PathBuf>) -> TempDir {
	Builder::new()
		...
		.tempdir_in(dir_path.as_ref().map(PathBuf::from).unwrap_or_else(|| {
			...
			if env_temp_dir.metadata().is_err() {
				DirBuilder::new()
					.mode(0o700)
					.create(&env_temp_dir)
					.unwrap_or_else(|e| {
				...
				let temp_dir_mode = env_temp_dir.metadata().unwrap().permissions().mode();
				assert_eq!(
...
mod tests {
	...
	fn test_create_with_dir_path() {
		...
		assert!(in_tmp_path, "directory: {:?}", tempdir.path());
		assert!(!in_tmp_subfolder, "directory: {:?}", tempdir.path());
	}
