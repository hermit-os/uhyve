...
// This implementation links [`UhyveVm`] & [`VirtualizationBackend`] to [`GdbVcpuManager`].
...
///
/// This is particularly necessary for musl, as [`Pthread`]
/// is an opaque pointer there which can't be passed to thread as easily
///
/// # Safety
///
/// This can be safely sent across threads because the thread-safety
/// of pthread IDs is ensured by the pthread library.
...
/// Shared state (between the GDBstub server thread and the CPU thread)
/// of a [`VcpuWrapper`], to avoid littering the code
/// with `Arc` wrap/unwrap operations.
...
/// # Safety
///
/// The `VCpu` doesn't get actually inserted into `VcpuWrapperShared` on MacOS,
/// (but gets only put into a `PhantomData` instead), therefore this is safe.
#[cfg(target_os = "macos")]
...
/// Data per each Vcpu for the GDBstub server.
...
/// The GDBstub server data, used to persist data necessary for the GDBstub state machine(s).
