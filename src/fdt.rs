...
pub struct Fdt {
	...
	maybe_hermit_image: Option<(u64, u64)>,
	kernel_args: String,
	app_args: String,
}
...
impl Fdt {
	...
	pub fn new(
		...
		let maybe_hermit_image = if let Some(hermit_image) = maybe_hermit_image.clone() {
	...
	pub fn finish(mut self) -> FdtWriterResult<Vec<u8>> {
		...
		if let Some((hermit_image_start, hermit_image_length)) = self.maybe_hermit_image {
			self.writer
