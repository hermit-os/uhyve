...
pub fn run_hello_world(c: &mut Criterion) {
	...
	let hello_world_path = [
		...
		"data/sample-v1-images/x86_64/hello_world",
	]
	.iter()
	.collect::<PathBuf>();
	assert!(
	...
	group.bench_function("uhyve data/sample-v1-images/x86_64/hello_world", |b| {
		b.iter(|| {
	...
	group.bench_function("qemu data/sample-v1-imags/x86_64/hello_world", |b| {
		b.iter(|| {
...
pub fn run_rusty_demo(c: &mut Criterion) {
	...
	group.bench_function("uhyve data/sample-v1-images/x86_64/rusty_demo", |b| {
		b.iter(|| {
			let status = Command::new(&uhyve_path)
				.arg(&rusty_demo_path)
				.stdout(Stdio::null())
	...
	group.bench_function("qemu data/sample-v1-images/x86_64/rusty_demo", |b| {
		b.iter(|| {
			let status = Command::new("qemu-system-x86_64")
				.arg("-smp")
				.arg("1")
